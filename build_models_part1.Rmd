...
pca_prefix <- if (USE_PCA) 'pca50_' else ''
...
```{r}
write.csv(result_BIC$coef, file = paste("models/coefs/", pca_prefix, "lasso_bic_coefs.csv", sep = ""), row.names = TRUE)
write.csv(result_EBIC$coef, file = paste("models/coefs/", pca_prefix, "lasso_ebic_coefs.csv", sep = ""), row.names = TRUE)

paste("models/coefs/", pca_prefix, "lasso_ebic_coefs.csv", sep = "")
```
...
g_ebic_pca <- 1/3.219938e-04


print("CV Lambda for normal surgests g of:")
print(g_cv)
print("Bic Lambda for PCA surgests g of:")
print(g_ebic_pca)
```
...
```{r}
g <- if (USE_PCA) g_ebic_pca else g_cv
...
```{r}
write.csv(bma_betas, file = paste("models/coefs/", pca_prefix, "bma_coefs.csv", sep = ""), row.names = TRUE)
write.csv(coef(fit.bayesreg), file = paste("models/coefs/", pca_prefix, "bma_coefs_intervalls.csv", sep = ""), row.names = TRUE)
